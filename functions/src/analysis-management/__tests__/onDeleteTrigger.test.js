...
// These mocks need to be defined *before* the jest.mock('firebase-admin', ...) call
...
const mockFirestoreServiceInstance_onDeleteTrigger = {
  ...
  // Helper to create snapshot-like objects for makeChange
...
jest.mock('firebase-admin', () => {
  ...
  return {
    ...
    firestore: firestoreFactoryMock_onDeleteTrigger, // Export the factory directly
...
describe('handleAnalysisDeletionRequest Firestore Trigger', () => {
  ...
  let functionsTestInstance;

  beforeAll(() => {
    functionsTestInstance = functionsTest({
      projectId: 'test-project-ondelete',
    });
    functionsTestInstance.mockConfig({
  ...
  beforeEach(() => {
    ...
    wrappedHandleAnalysisDeletionRequest = functionsTestInstance.wrap(
  ...
  const createChangeObject = (beforeData, afterData) => {
    ...
    return functionsTestInstance.makeChange(beforeSnap, afterSnap);
  };
  ...
  it('should log critical error if Firestore update fails after successful file deletions', async () => {
    ...
    currentFirestoreDocRefMethods_onDeleteTrigger.update.mockImplementationOnce(async (payload) => {
      // @ts-expect-error - Test: Simulating payload for status check
