    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          O relatório estruturado foi gerado. Você pode visualizar um resumo abaixo, baixar o
          conteúdo completo ou ver o relatório detalhado em uma nova página.
        </p>
      </div>
        ...
        <Card className="bg-muted/30">
          <CardHeader>
            <CardTitle className="text-lg">
              {structuredReport.reportMetadata?.title || 'Relatório de Conformidade'}
            </CardTitle>
            {structuredReport.reportMetadata?.subtitle && (
              <CardDescription>{structuredReport.reportMetadata.subtitle}</CardDescription>
            )}
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              ...
              <p className="text-sm whitespace-pre-wrap text-foreground/80">
                ...
                {structuredReport.introduction?.objective || 'Não disponível.'}
              </p>
              <p className="text-sm whitespace-pre-wrap mt-1 text-foreground/80">
                <strong>Resumo dos Resultados:</strong>{' '}
                {structuredReport.introduction?.overallResultsSummary || 'Não disponível.'}
              </p>
            </div>
            ...
            {structuredReport.analysisSections && structuredReport.analysisSections.length > 0 && (
              <div>
                ...
                <Accordion type="single" collapsible className="w-full max-h-60 overflow-y-auto">
                  {structuredReport.analysisSections.slice(0, 3).map(
                    (
                      section,
                      index // Mostrar prévia de até 3 seções
                    ) => (
                      <AccordionItem
                        value={`section-${index}`}
                        key={index}
                ...
                {structuredReport.analysisSections.length > 3 && (
                  <p className="text-xs text-muted-foreground mt-1">
                    Mais seções disponíveis no relatório detalhado.
                  </p>
                )}
              </div>
            )}
      ...
      <div className="flex flex-wrap gap-2 mt-4">
        ...
        <Button onClick={downloadJsonReport} variant="outline" disabled={!structuredReport}>
