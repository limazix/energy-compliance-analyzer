...
// Mock firebase-admin for retryAnalysisAction
...
// Mock firebase/functions for processAnalysisFile
...
describe('Analysis Processing Server Actions (Unit)', () => {
  ...
  describe('processAnalysisFile', () => {
    ...
    it('should handle cases where analysisId is null or undefined', async () => {
      // @ts-expect-error - Testing invalid input, analysisId cannot be null for this action.
      ...
      // @ts-expect-error - Testing invalid input, analysisId cannot be undefined for this action.
  ...
  describe('retryAnalysisAction', () => {
    ...
    it('should return an error if powerQualityDataUrl is missing', async () => {
      mockAdminDocGet.mockResolvedValueOnce({
        exists: true,
        data: () => ({ status: 'error', powerQualityDataUrl: null }),
      });
