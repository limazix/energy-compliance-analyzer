...
describe('File Upload Server Actions (Unit)', () => {
  ...
  describe('createInitialAnalysisRecordAction', () => {
    it('should call httpsCreateInitialAnalysisRecord and return analysisId and signedUrl', async () => {
      const mockSignedUrl = 'https://fake.url/upload';
      const mockResponse: HttpsCallableResult<{ analysisId: string; signedUrl: string }> = {
        data: { analysisId: MOCK_ANALYSIS_ID, signedUrl: mockSignedUrl },
      };
      mockCallableFn.mockResolvedValueOnce(mockResponse);
      const result = await createInitialAnalysisRecordAction(
        MOCK_USER_ID,
        MOCK_FILE_NAME,
        'Test Title',
        'Test Desc',
      ...
      expect(mockCallableFn).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          languageCode: 'en-US',
          // fileSize and fileType would be part of the actual call if this action handled it
        })
      );
