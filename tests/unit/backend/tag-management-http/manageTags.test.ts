...
} from '@functions/tag-management-http/manageTags';
...
const _MAX_ERROR_MESSAGE_LENGTH = APP_CONFIG.MAX_SERVER_ERROR_MESSAGE_LENGTH; // Not used, but kept for context
...
describe('Tag Management HTTPS Callable Functions (Unit)', () => {
  ...
  describe('httpsCallableAddTag', () => {
    ...
    const validAuthContext = { auth: { uid: MOCK_USER_ID } } as functions.https.CallableContext;

    it('should throw "unauthenticated" if no auth context', async () => {
      // @ts-expect-error - Testing invalid context: unauthenticated user
      await expect(
        httpsCallableAddTag(validData, {} as functions.https.CallableContext)
    ...
    it('should throw "invalid-argument" if analysisId is missing', async () => {
      ...
      await expect(
        httpsCallableAddTag({ tag: 'tag1' } as { tag: string }, validAuthContext)
      ).rejects.toMatchObject({
