// functions/src/analysis-management/__tests__/onDeleteTrigger.test.ts
'use strict';

/**
 * @fileOverview Test suite for the handleDeleteTrigger Firestore-triggered Firebase Function.
 */
...
const mockFirestoreServiceInstance_onDeleteTrigger = {
  ...
  snapshot_: jest.fn((data: unknown, docPath: string) => ({
    data: () => data,
...
Object.assign(firestoreFactoryMock_onDeleteTrigger, {
  ...
  FieldValue: {
    ...
    arrayUnion: jest.fn((...args: unknown[]) => ({ _methodName: 'arrayUnion', _elements: args })),
    arrayRemove: jest.fn((...args: unknown[]) => ({ _methodName: 'arrayRemove', _elements: args })),
    delete: jest.fn(() => ({ _methodName: 'delete' })),
    increment: jest.fn((n: number) => ({ _methodName: 'increment', _operand: n })),
  },
});
...
jest.mock('firebase-admin', () => {
  ...
  const createMockAppInternal = (name: string): adminActual.app.App =>
    ({
      name,
      firestore:
        firestoreFactoryMock_onDeleteTrigger as unknown as () => adminActual.firestore.Firestore, // Use the factory mock defined above
      storage: jest.fn(() => ({
        bucket: jest.fn(() => ({
          file: jest.fn(() => ({
            exists: jest.fn(),
...
describe('handleAnalysisDeletionRequest Firestore Trigger', () => {
  ...
  const createChangeObject = (
    ...
    const beforeSnap = mockFirestoreServiceInstance_onDeleteTrigger.snapshot_(
      beforeData,
      docPath
    ) as functions.firestore.DocumentSnapshot;
    const afterSnap = mockFirestoreServiceInstance_onDeleteTrigger.snapshot_(
      afterData,
      docPath
    ) as functions.firestore.DocumentSnapshot;
    return testInstance.makeChange(
      beforeSnap,
      afterSnap
    ) as functions.Change<functions.firestore.DocumentSnapshot>;
  };

  const context = {
    params: { userId: MOCK_USER_ID, analysisId: MOCK_ANALYSIS_ID },
  } as unknown as functions.EventContext; // Cast for type safety
  ...
  it('should update status to "error" if CSV deletion fails', async () => {
    ...
    mockDeleteAdminFileFromStorage.mockImplementation(async (filePath: string) => {
  ...
  it('should log critical error if Firestore update fails after successful file deletions', async () => {
    ...
    currentFirestoreDocRefMethods_onDeleteTrigger.update.mockImplementationOnce(
