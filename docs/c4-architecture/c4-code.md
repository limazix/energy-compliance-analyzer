...
C4Dynamic
  ...
  Person(user, "Usuário", "Interage com a UI do chat.")
  Component(reportPageUI, "ReportPage UI (React)", "Componente da página de relatório que gerencia o chat.")
  Component(reportChatActions, "`reportChatActions.ts` (Server Action)", "Ponto de entrada no backend para o chat.")
  Component(orchestrationFlow, "`orchestrateReportInteractionFlow` (Genkit Flow)", "Fluxo Genkit que usa IA para entender e responder ao usuário, podendo usar ferramentas.")
  Component(revisorTool, "`callRevisorTool` (Genkit Tool)", "Ferramenta Genkit que invoca o fluxo `reviewComplianceReportFlow` para modificar o relatório estruturado.")
  Component(reviewFlow, "`reviewComplianceReportFlow` (Genkit Flow)", "Fluxo Genkit para revisar o relatório estruturado.")
  ContainerDb(rtdb, "Firebase Realtime Database", "Armazena mensagens do chat.")
  ContainerDb(firestore, "Firebase Firestore", "Armazena o relatório estruturado (JSON).")
  Container(storage, "Firebase Storage", "Armazena o relatório MDX.")
  System_Ext(googleAI, "Google AI (Gemini)", "Modelo de Linguagem para Genkit.")
...
C4Dynamic
  ...
  System_Ext(firestoreTrigger, "Gatilho Firestore", "`onUpdate('users/{userId}/analyses/{analysisId}')` quando status é 'summarizing_data'")
  Component(processAnalysisFn, "`processAnalysis.js`", "Função principal orquestradora da pipeline de análise em Firebase Functions.")
  Component(gcsReader, "`getFileContentFromStorage`", "Lê conteúdo do arquivo CSV do Firebase Storage.")
  Component(summarizeDataFlow, "`summarizePowerQualityDataFlow`", "Fluxo Genkit (usa `summarize-power-quality-data.ts`). Sumariza CSV.")
  Component(identifyResolutionsFlow, "`identifyAEEEResolutionsFlow`", "Fluxo Genkit (usa `identify-aneel-resolutions.ts`). Identifica resoluções.")
  Component(analyzeReportFlow, "`analyzeComplianceReportFlow`", "Fluxo Genkit (usa `analyze-compliance-report.ts`). Gera relatório estruturado JSON.")
  Component(reviewReportFlow, "`reviewComplianceReportFlow`", "Fluxo Genkit (usa `review-compliance-report.ts`). Revisa relatório JSON.")
  Component(mdxConverter, "`convertStructuredReportToMdx`", "Utilitário (`reportUtils.ts`). Converte JSON para MDX.")
  System_Ext(googleAI, "Google AI (Gemini)", "Modelo de Linguagem para Genkit.")
  ContainerDb(storage, "Firebase Storage", "Armazena CSVs e relatórios MDX.")
  ContainerDb(firestore, "Firebase Firestore", "Armazena metadados, status, e relatório JSON.")
